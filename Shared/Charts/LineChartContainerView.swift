...
class LineChartContainerViewModel<X: Hashable & Comparable>: ObservableObject {
	...
	init(dataSources: [ChartData<X>] = [], fixedSize: CGSize? = nil) {
		...
		let dataSourcesChangeSubscription = dataSources.publisher.sink { value in
	...
	func refineMarginValue() {
		...
		let refinedY3 = scaleFunctionX(0) - labelMargin - String(format: "%.2f", yTick.stop
		).widthOfString(usingFont: axisLabelTraditionalFont) // Y 轴最上方标签

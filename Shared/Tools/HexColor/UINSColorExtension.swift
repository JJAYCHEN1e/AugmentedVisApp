...
@objc extension UINSColor {
    ...
    public convenience init(rgba_throws rgba: String) throws {
        guard rgba.hasPrefix("#") else {
            let error = HexColorInputError.missingHashMarkAsPrefix(rgba)
        ...
        guard Scanner(string: hexString).scanHexInt32(&hexValue) else {
            let error = HexColorInputError.unableToScanHexValue(rgba)
            print(error.localizedDescription)
            throw error
        }
        ...
        default:
            let error = HexColorInputError.mismatchedHexStringLength(rgba)
            print(error.localizedDescription)
            throw error
        }
    }
    ...
    public func hexStringThrows(_ includeAlpha: Bool = true) throws -> String  {
        ...
        guard r >= 0 && r <= 1 && g >= 0 && g <= 1 && b >= 0 && b <= 1 else {
            let error = HexColorInputError.unableToOutputHexStringForWideDisplayColor
            print(error.localizedDescription)
            throw error
        }
